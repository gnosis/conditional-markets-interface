...
contract("Oracles", function(accounts) {
  ...
  before(async () => {
    ...
    // Condition IDs
    conditionOneId = keccak256(
      DifficultyOracle.address +
        [
    ...
    conditionTwoId = keccak256(
      GasLimitOracle.address +
        [
    ...
    // LMSR Address
    checksummedLMSRAddress = toChecksumAddress(
      keccak256(rlp.encode([lmsrFactory.address, 0x01])).substr(26)
    );
    ...
    positionId1 = keccak256(collateralToken.address + collectionId1.slice(2));
    positionId2 = keccak256(collateralToken.address + collectionId2.slice(2));
    positionId3 = keccak256(collateralToken.address + collectionId3.slice(2));
    positionId4 = keccak256(collateralToken.address + collectionId4.slice(2));
  });
  ...
  it("The LMSR should have the correct amount of tokens at the specified positions", async () => {
    assert.equal(
      await pmSystem.balanceOf(lmsrInstance.address, positionId1),
      process.env.AMMFUNDING
    );
    assert.equal(
      await pmSystem.balanceOf(lmsrInstance.address, positionId2),
      process.env.AMMFUNDING
    );
    assert.equal(
      await pmSystem.balanceOf(lmsrInstance.address, positionId3),
      process.env.AMMFUNDING
    );
    assert.equal(
      await pmSystem.balanceOf(lmsrInstance.address, positionId4),
      process.env.AMMFUNDING
    );
  });
  ...
  it("Users should be able to make complex buy / sell orders", async () => {
    ...
    await lmsrInstance.trade([-1e9, -1e9, -1e9, -1e9], toBN(1e18), {
      from: accounts[9]
    });
  ...
  it("Oracles should be able to send results", async () => {
    ...
    truffleAssert.eventNotEmitted(
      ...
      "resolutionFailed",
      null,
      "Event resolution shouldn't fail"
    );
    truffleAssert.eventNotEmitted(
      resolveDiff,
      "resolutionFailed",
  ...
  it("Users should be able to redeem their positions now, but not buy more", async () => {
    const beforeRedeemBalance = await collateralToken
      .balanceOf(accounts[9])
      .then(r => r.toString());
    ...
    assert.equal(
      await collateralToken.balanceOf(accounts[9]).then(r => r.toString()),
      beforeRedeemBalance
    );
