...
const writeToConfig = (network, { conditionIds, lmsr, collateral }) => {
  ...
  if (networkConfig == null) {
    console.warn(
      `Key does not exist. Adding the network key ${networkName} to the config`
    );
  ...
  if (conditionIds) {
    if (networkConfig.markets.length !== conditionIds.length) {
      console.warn(
        `Wrong amount of markets in config, ${
          conditionIds.length
        } conditions generated, ${networkConfig.markets.length} markets in file`
      );
